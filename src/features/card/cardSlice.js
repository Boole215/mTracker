...
import { mangadexAPI }  from "../client/mangadexAPI";
...
export const fetchSeriesByID = createAsyncThunk(
    ...
    async (seriesID) => {
        ...
        let mangaResponse = await mangadexAPI.fetchManga(seriesID)

        console.log("requesting chapter info")
        const chapterResponse = await mangadexAPI.fetchChap(seriesID)
        //console.log("requesting chapter info")
        ...
        const coverResponse = await mangadexAPI.fetchCover(mangaResponse["relationships"]["2"]["id"])
        ...
        mangaResponse["data"]["coverURL"] = `https://uploads.mangadex.org/covers/${mangaResponse["data"]["id"]}/${coverResponse["data"]["attributes"]["fileName"]}.512.jpg`
        mangaResponse["data"]["chapterList"] = chapterResponse.results
...
export const feedCardSlice = createSlice({
    ...
    extraReducers:{
        [fetchSeriesByID.fulfilled]: (state, action) => {
        ...
        [fetchSeriesByID.rejected]: (state, action) =>{
